...
For deployment facts (URLs, region, IAM) see
`deploy-cheatsheet.md`. This file is the
architecture spec.
...
graph LR
    ...
    subgraph GCP["Google Cloud Platform — <gcp-project>"]
        subgraph LB["External HTTPS Load Balancer"]
            LBFR["Forwarding rule<br/><deployment-ip>"]
        ...
        Registry["Artifact Registry<br/>gcr.io/<gcp-project>/expense-reports"]
        Firestore["Firestore (default db)<br/>jobs/{upload_id}<br/>reports/{upload_id}"]
        GCS["Cloud Storage<br/><bucket-name><br/>uploads/{id}/files + extractions"]
...
- Storage under `/app/.scratch/uploads/` is a **disk cache**, not authoritative. A container recycle wipes it, but the durable tier (Firestore + GCS) survives — Rehydrate repopulates the cache on the next request. The "upload → review immediately" workflow remains the common case; the rehydrate path makes 24-hour-old URLs work after a recycle.
- **Durable store (2026-05-25 → 2026-05-26)**, gated by three independent env vars set in `deploy/cloudbuild.yaml`:
  - **`USE_FIRESTORE_JOBS=1`** — `_set_job` / `_get_job` in `scripts/local_app_simple.py` dispatch to `scripts/firestore_jobs.py` (`jobs/{upload_id}`, 7-day TTL). Container recycle mid-upload no longer wipes the SSE stream.
  - **`USE_FIRESTORE_REPORTS=1`** — every pipeline completion + every edit/delete/undo dual-writes to `scripts/firestore_reports.py` (`reports/{upload_id}`, 90-day TTL). Stores `{report_json, fa_input, history}`. Report payload is JSON-encoded as a string to dodge Firestore's "invalid nested entity" rejection of arrays-of-arrays (bbox coordinates).
  - **`USE_GCS_ARTIFACTS=1`** — source PDFs + per-receipt extraction JSONs dual-write via `scripts/gcs_artifacts.py` to `gs://<bucket-name>/uploads/{id}/{files,extractions}/`.
...
| Persist JOBS / reports / source artifacts across container recycle | `scripts/firestore_jobs.py` + `scripts/firestore_reports.py` + `scripts/gcs_artifacts.py` + `_rehydrate_upload` in `scripts/local_app_simple.py` | Durable store |
...
| Generated response_schemas (per call) | `generated/response_schema_meal_main.json` + `_extras.json`, `generated/response_schema_transport_main.json` + `_extras.json`, `generated/response_schema_lodging_main.json` + `_extras.json`, `generated/response_schema_conference_registration_main.json` + `_extras.json`, `generated/response_schema_airfare_main.json` + `_aux.json` + `_extras.json`, plus single-call `generated/response_schema_miscellaneous.json`, `generated/response_schema_membership.json`, and `generated/response_schema_mileage.json` . (meal/transport/lodging/conference_registration are split across 2 parallel calls; airfare across 3 — see §1 and §7) |
...
| Per-kind Python extractors (Gemini call) | `scripts/extract_meal.py` (2 parallel calls — main carries common+meal_details; extras carries merchant_address+printed_currency), `scripts/extract_transport.py` (same 2-call pattern), `scripts/extract_lodging.py` (2 parallel calls; extras includes per-night rate breakdown), `scripts/extract_airfare.py` (3 parallel calls — main/aux/extras — and 1-deep-merges `airfare_details` from main+aux), `scripts/extract_conference_registration.py` (2 parallel calls — main carries common+conference_registration_details; extras carries merchant_address+printed_currency), `scripts/extract_miscellaneous.py` (posters/printing/etc., no detail block, single-call), `scripts/extract_membership.py` (ACM/IEEE/USENIX dues, no detail block, single-call), `scripts/extract_mileage.py` (personal mileage from Google Maps screenshot or driving log, single-call) |
| Shared extractor infrastructure | `scripts/extractor_lib.py` (CLI parsing, ADC client, `single_call` primitive used by all multi-call extractors, `run_extraction` wrapper for single-call kinds like miscellaneous/membership/mileage, retry-with-backoff on Vertex 5xx) |
| External-data scripts (manual periodic refresh) | `scripts/fetch_irs_mileage_rate.py` (scrapes IRS standard mileage rates → `generated/irs_mileage_rates.json`; run when IRS announces a rate change). `scripts/generate_airport_codes.py` (Stanford-side airport-code lookup table). Both follow the same pattern: manual fetch → JSON cache committed to git → consumed at runtime with no network dep. |
...
| Removed historical spikes | (removed) |
|||
