...
use expense_report_schema::{
    ...
    transcribe_document_path, transcribe_document_path_with_document_ai_profile,
    transcribe_document_path_with_vertex, transcribe_document_path_with_vertex_sdk_profile,
    DocumentAiConfig, DocumentAiPassProfile, OcrPassKind, OcrPreprocessVariant, VertexGeminiConfig,
...
fn run() -> Result<(), String> {
    ...
    let document =
        match engine.as_str() {
            ...
            _ => return Err(
                "engine must be one of builtin | vertex-gemini | vertex-gemini-sdk | document-ai"
                    .to_owned(),
            ),
        };
